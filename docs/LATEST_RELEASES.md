...
初期表示・初期HTMLは直近30日分だけにする。ブラウザ上で次のフィルタを行う。
...
90日または1年を選んだ場合だけ、ビルド済みの同一オリジン静的JSON `/releases/data.json` を読み込む。このJSONには31〜365日前の対象系列だけを含める。外部APIやWorker処理は発生しない。
...
そこで「最新リリース」という用途では直近1年を探索上限とし、さらに通常アクセスで必要な直近30日だけをHTMLへ含める。31〜365日前はユーザーが期間を広げた時だけ静的JSONから読み込む。これによりSEO向けの最近の情報をHTMLに残しつつ、初期転送量を抑える。
...
- canonical: `/releases/`
- index対象の通常静的ページ
- 直近30日の情報は静的HTMLとして検索エンジンから読める状態を維持
...
初回の全期間版・365日HTML版の実測をもとに、初期HTMLを30日へ限定した構成で再計測し、実サイズへ適正な余裕を持たせた固定予算へ締める。
