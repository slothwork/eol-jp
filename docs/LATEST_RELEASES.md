...
初期表示は直近30日。直近1年分のデータを静的HTMLに含め、ブラウザ上で次のフィルタを行う。
...
JavaScriptが無効な場合は直近1年分を絞り込まず表示する。

## なぜ全期間を表示しないか

初回実装で全サポート系列をHTMLへ含めたところ、Production buildの `/releases/` は raw HTML 約1.34MB / gzip相当約78.5KiBとなった。通常利用で必要性の低い古いlatestまで初期転送するのは、サイトのperformance budget方針と合わない。

「最新リリース」という用途では直近1年を十分な探索範囲とし、それ以前の系列は既存の製品詳細ページに委ねる。これにより、追加APIやクライアント側データ取得を導入せず静的サイトの単純性も維持する。
...
初回の全期間版で実測した結果をもとに直近365日へ対象を限定し、再計測後に実サイズへ適正な余裕を持たせた固定予算へ締める。
