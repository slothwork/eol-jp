...
} from './external-notification-security.ts';
import type { WorkerKvNamespace, WorkerRuntimeEnv } from './runtime-types.ts';
...
function externalNotificationsConfigured(env: WorkerRuntimeEnv): boolean {
  return Boolean(env.NOTIFICATION_SUBSCRIPTIONS && env.TURNSTILE_SITE_KEY && env.TURNSTILE_SECRET_KEY);
}

function externalNotificationConfig(env: WorkerRuntimeEnv): Response {
  return json({
...
async function loadSubscription(kv: WorkerKvNamespace, id: string): Promise<NotificationSubscription | null> {
...
async function createSubscription(request: Request, env: WorkerRuntimeEnv): Promise<Response> {
...
async function getSubscription(request: Request, env: WorkerRuntimeEnv, id: string): Promise<Response> {
  const kv = env.NOTIFICATION_SUBSCRIPTIONS;
  if (!kv) return json({ error: 'notification_storage_unconfigured' }, 503);
...
async function updateSubscription(request: Request, env: WorkerRuntimeEnv, id: string): Promise<Response> {
  const kv = env.NOTIFICATION_SUBSCRIPTIONS;
  if (!kv) return json({ error: 'notification_storage_unconfigured' }, 503);
  if (!sameOriginRequest(request)) return json({ error: 'forbidden_origin' }, 403);
  const subscription = await loadSubscription(kv, id);
...
async function deleteSubscription(request: Request, env: WorkerRuntimeEnv, id: string): Promise<Response> {
  const kv = env.NOTIFICATION_SUBSCRIPTIONS;
  if (!kv) return json({ error: 'notification_storage_unconfigured' }, 503);
  if (!sameOriginRequest(request)) return json({ error: 'forbidden_origin' }, 403);
  const subscription = await loadSubscription(kv, id);
...
export async function handleExternalNotificationApi(request: Request, env: WorkerRuntimeEnv): Promise<Response> {
...
export async function runExternalNotifications(
